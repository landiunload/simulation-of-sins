...
- `ЛКМ` — убрать блок под прицелом, `ПКМ` — достроить соседний;
- `Shift` + `ЛКМ` — импульс физическим телам вокруг точки попадания;
- `F7` — сменить D3D12/Vulkan, если доступны оба бэкенда;
- `Escape` — выход.

По умолчанию спавнер создаёт ровно **1000 Г-образных тел по 9 блоков**:
100 тел в секунду, 10 секунд физического времени. Затем спавн прекращается,
а физика продолжает работать. Тела не удаляются и не заменяются новыми после
разрушения. Начальная раскладка — сетка 10×10 с шагом 8 блоков и десять
высотных слоёв; это исключает неизбежное наложение больших букв при спавне
в одной точке. Постройки можно достраивать и разбирать.
Связные блоки — одно вращающееся тело; после разрыва связи компоненты становятся
отдельными телами. Постройки и кубы сталкиваются в общем fixed-step решателе
движка, без отдельной покадровой AABB-симуляции. Правки и импульсы применяются
перед следующим физическим тиком. Для воспроизведения нужны одинаковые начальные
данные, seed, настройки решателя и входные команды на одинаковых тиках.
